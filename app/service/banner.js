...
class BannerService extends Service {
  ...
  // 获取所有数据
  async getList() {
    const result = await this.app.mysql.select('home_slide', {
      orders: [[ 'id', 'desc' ]], // 排序方式
    });
    return { data: result };
  }
  ...
  // 根据用户id查询数据
  async findByID(id) {
    const result = await this.app.mysql.get('home_slide', { id });
    return { data: result };
  }
  ...
  async addModel(data) {
    ...
    // 新增数据
    const result = await app.mysql.insert('home_slide', data);
    return {
  ...
  async updateModel(data) {
    ...
    const { ctx, app } = this;
    data.createtime = ctx.helper.currentDateTime();
    const result = await app.mysql.update('home_slide', data);
    return {
      error_code: result.affectedRows > 0 ? 0 : 1,
  ...
  async destroyModel(id) {
    const result = await this.app.mysql.delete('home_slide', { id });
    return {
      error_code: result.affectedRows > 0 ? 0 : 1,
